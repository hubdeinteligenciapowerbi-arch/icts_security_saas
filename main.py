...
def carregar_e_preparar_dados():
    ...
    df = df[~df['bairro'].isin(junk_geral) & ~df['bairro'].str.match(r'^\d+$') & ~df['bairro'].str.match(r'^\d{5}-\d{3}$') & ~df['bairro'].str.match(r'^\(.*\)$') & (df['bairro'].str.len() > 2)]
    df = df[~df['municipio'].isin(junk_geral) & (df['municipio'].str.len() > 2)]
    df = df[~df['regiao'].isin(junk_geral) & (df['regiao'].str.len() > 2)]
    ...
    df = df[(df['latitude'].between(SP_LAT_MIN, SP_LAT_MAX)) & (df['longitude'].between(SP_LON_MIN, SP_LON_MAX))]
    
    crimes_validos = ['fios e cabos', 'joalheria', 'lesao corporal de natureza grave', 'morte acidental', 'morte subita, sem causa determinante aparente', 'pessoa', 'residencia', 'saidinha de banco', 'transeunte', 'veiculo']
...
def get_ssp_locais_df():
    ...
    try:
        ...
        if not dados_api: raise ValueError("A API da SSP retornou uma lista vazia.")
        df = pd.DataFrame(dados_api)
        if 'NOME_SECCIONAL' not in df.columns: raise ValueError("A resposta da API da SSP não contém a coluna 'NOME_SECCIONAL'.")
...
app = FastAPI(title="API de Dados de Segurança Pública (Unificada)", description="Fornece dados e insights sobre ocorrências criminais.", version="6.0.0")
...
def get_filtered_data(periodo, regiao, municipio, bairro, delito):
    ...
    if periodo == 'last_30_days': df_filtrado = df_filtrado[df_filtrado['data_registro'] >= (data_maxima - timedelta(days=30))]
    elif periodo == 'last_quarter': df_filtrado = df_filtrado[df_filtrado['data_registro'] >= (data_maxima - timedelta(days=90))]
    elif periodo == 'all_2025': df_filtrado = df_filtrado[df_filtrado['ano'] == 2025]
    if regiao and regiao.lower() != 'string': df_filtrado = df_filtrado[df_filtrado["regiao"] == normalizar_str(regiao)]
    if municipio and municipio.lower() != 'string': df_filtrado = df_filtrado[df_filtrado["municipio"] == normalizar_str(municipio)]
    if bairro and bairro.lower() != 'string': df_filtrado = df_filtrado[df_filtrado["bairro"] == normalizar_str(bairro)]
...
def get_insights(request: InsightsRequest):
    ...
    try:
        ...
        if df_filtrado.empty:
            return {"quantidade_total": 0, "detalhamento_ocorrencias": [], "analise_curta": "Não há ocorrências para os filtros selecionados.", "recomendacao_curta": "Ajuste os filtros para uma nova busca."}
        ...
        if request.bairro and request.bairro.lower() != 'string': local = f"o bairro {request.bairro.title()}"
        elif request.municipio and request.municipio.lower() != 'string': local = f"o município de {request.municipio.title()}"
        elif request.regiao and request.regiao.lower() != 'string': local = f"a região de {request.regiao.title()}"
        ...
        prompt = (f"Aja como um analista de segurança pública. Com base nos dados a seguir, gere um objeto JSON contendo apenas duas chaves: 'analise_curta' e 'recomendacao_curta'. Sua resposta DEVE ser apenas o objeto JSON, sem nenhum texto ou formatação adicional.\n\n## DADOS PARA ANÁLISE:\n- Local: {local}\n- Período: {periodo_str}\n- Total de Ocorrências: {total}\n- Detalhamento dos Delitos: {json.dumps(detalhamento_lista, ensure_ascii=False)}\n\n## FORMATO DE SAÍDA ESPERADO (APENAS O JSON):\n{{\n  \"analise_curta\": \"Gere uma análise de uma frase sobre o cenário criminal, destacando o principal delito.\",\n  \"recomendacao_curta\": \"Gere uma recomendação de segurança de uma frase, curta e acionável para os cidadãos.\"\n}}")
        body = {"contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.6, "maxOutputTokens": 1024, "response_mime_type": "application/json"}}
        ...
        resposta_final = {"quantidade_total": total, "detalhamento_ocorrencias": detalhamento_lista, "analise_curta": analise_parcial.get("analise_curta", "Análise não pôde ser gerada."), "recomendacao_curta": analise_parcial.get("recomendacao_curta", "Recomendação não pôde ser gerada.")}
...
def ocorrencias(periodo: str = Query("last_quarter", enum=["last_30_days", "last_quarter", "all_2025"]), municipio: str = Query(None), regiao: str = Query(None), bairro: str = Query(None), delito: str = Query(None)):
    try:
